...
#
# Every cycle walks the live endpoints below, so a short interval multiplies
# into a lot of HTTP requests at the inverter.  60 seconds keeps the load at
# four requests per minute while still being responsive enough for the energy
# dashboard; users who want faster updates can lower it in the options flow.
DEFAULT_SCAN_INTERVAL = 60
MIN_SCAN_INTERVAL = 15
MAX_SCAN_INTERVAL = 3600

# Endpoints holding static device identity data.  Fetched once during the first
# refresh and then served from the coordinator cache.
STATIC_ENDPOINTS = [
    "devices",
]

# Endpoints holding live data.  Fetched on every polling cycle.
POLLED_ENDPOINTS = [
    "alarms",
    "energies",
    "modes",
...
# Full list of endpoints, used for the initial refresh
GET_ENDPOINTS = STATIC_ENDPOINTS + POLLED_ENDPOINTS
