...
async def async_setup_entry(
    ...
    modes_time_config_text = APsystemsStorageModesTimeSlotsText(
        coordinator=coordinator, entry_id=entry.entry_id
    )
    controlpanels_config_text = APsystemsStorageControlPanelsTimeSlotsText(
        coordinator=coordinator, entry_id=entry.entry_id
    )
    ...
    async_add_entities([modes_time_config_text, controlpanels_config_text])
...
class APsystemsStorageModesTimeSlotsText(
    ...
    def name(self) -> str | None:
        ...
        would hide the text entity's own label such as "Time Configuration".
        We always return a real name from ``entity_description.name`` so the
        UI shows ``"APsystems Storage Time Configuration"`` (with ``" *"``
    ...
    def native_value(self) -> str:
        ...
        time_cfg = self.staged_api_value
        if time_cfg is None:
            return "[]"

        return json.dumps(time_cfg, ensure_ascii=False)

    @property
    def staged_api_value(self) -> list[dict[str, Any]] | None:
        """Return the value to post to the device, or None if unknown.

        The parsed object is returned, so the button does not have to re-parse
        the entity's state string (which is also length-capped by ``native_max``).
        """
        if self._pending_modes_time_config is not None:
            return self._pending_modes_time_config

        time_cfg = endpoint_field(self.coordinator, "modes", "time_cfg")
        if isinstance(time_cfg, list) and time_cfg:
            return time_cfg
...
class APsystemsStorageControlPanelsTimeSlotsText(
    ...
    def name(self) -> str | None:
        ...
        Returning ``None`` here would let Home Assistant fall back to the
        device name alone (because ``_attr_has_entity_name`` is True), which
        would hide the text entity's own label such as "Control Panel
        Configuration". We always return a real name from
        ``entity_description.name`` so the UI shows
        ``"APsystems Storage Control Panel Configuration"`` (with ``" *"``
        while a change is staged).
        """
    ...
    @property
    def staged_api_value(self) -> dict[str, Any] | None:
        """Return the value to post to the device, or None if unknown.
        ...
        """
        if self._pending_controlpanels_config is not None:
            return self._pending_controlpanels_config

        config = endpoint_field(self.coordinator, "control-panels", "MI1")
