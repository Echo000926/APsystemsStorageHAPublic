...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    # Options win over the original setup data, so that changing the port or the
    # polling interval in the options flow actually takes effect.
    ...
    coordinator = APsystemsStorageCoordinator(
        hass, api_client, scan_interval=scan_interval
    )
    ...
    # Reload when the user changes options, so the new port / polling interval
    # is picked up without restarting Home Assistant.
    entry.async_on_unload(entry.add_update_listener(_async_update_listener))

    _LOGGER.info(
        "APsystems Storage integration successfully set up for %s:%s "
