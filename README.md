...
| 配置写入失败   | • JSON 格式错误（尤其 Time Configuration）<br>• 设备忙（正在处理其他请求）<br>• Token 过期（部分固件需认证） | 1. 使用 JSONLint 校验 Text 实体内容<br>2. 等待 30 秒后重试<br>3. 查看 HA 日志中 `apsystems_storage` 的 400 Bad Request 响应详情 |
