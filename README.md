...
- ⚙️ **Advanced Configuration Management**: Allows direct modification of operating modes, Depth of Discharge (DoD), and backup charging power within Home Assistant.
...
| Polling Interval | How often live data is refreshed (seconds). Range 15–3600 s. Lower values update faster but increase device API load. | 60 |
...
The polling interval can be changed at any time without reconfiguring the connection. Navigate to the integration page → Click the three-dot menu → Select **Configure**. The **Polling Interval** field (in seconds) accepts values from **15** to **3600** and defaults to **60**. After saving, the integration reloads automatically and applies the new interval. A higher interval reduces device API load; a lower interval provides fresher data.
...
1.  **Data Collection**: The Coordinator polls the device's local HTTP API on a configurable interval (default **60 seconds**, adjustable between **15 s and 3600 s** in the integration's options). The static `/devices` endpoint is fetched only once and cached; on every subsequent poll cycle only the live endpoints `/alarms`, `/energies`, `/modes`, and `/control-panels` are queried. At the default 60 s interval this amounts to roughly **4 requests per minute** to the device.
